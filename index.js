...
var SKIP, all, asSeq, changedBetween, empty, filterM, git, join, log, logSeq, map, previousCommitsSeq, produced, reduced, refTreeEntry, resolve, series, treeEntry, window, _ref, _ref1;
...
refTreeEntry = function(ref, path) {
  return ref.tree().then(function(tree) {
...
changedBetween = function(path, commit, prevCommit) {
  if (prevCommit != null) {
    return all(refTreeEntry(commit, path), refTreeEntry(prevCommit, path)).then(function(_arg) {
